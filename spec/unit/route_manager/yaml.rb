...
describe Puppet::RouteManager::Yaml, " when choosing file location" do
    ...
    describe Puppet::RouteManager::Yaml, " when storing objects as YAML" do
        ...
        it "should create the router subdirectory if it does not exist" do
            yaml = @subject.to_yaml
            file = mock 'file'
            path = @store.send(:path, @subject.name)
    ...
    describe Puppet::RouteManager::Yaml, " when retrieving YAML" do
    ...
    describe Puppet::RouteManager::Yaml, " when searching" do
      it "should return an array of fact instances with one instance for each file when globbing *" do
        ...
        Dir.expects(:glob).with(File.join("/my/yaml/dir", @store.class.router_name.to_s, @request.key)).returns(%w{one.yaml two.yaml})
      ...
      it "should return an array containing a single instance of fact when globbing 'one*'" do
        ...
        @one = mock 'one'
        @store.expects(:base).returns "/my/yaml/dir"
        Dir.expects(:glob).with(File.join("/my/yaml/dir", @store.class.router_name.to_s, @request.key)).returns(%w{one.yaml})
        YAML.expects(:load_file).with("one.yaml").returns @one;
      ...
      it "should return an empty array when the glob doesn't match anything" do
        ...
        @store.expects(:base).returns "/my/yaml/dir"
        Dir.expects(:glob).with(File.join("/my/yaml/dir", @store.class.router_name.to_s, @request.key)).returns([])
