...
describe Puppet::RouteManager::Request do
    describe "when initializing" do
        it "should require an router name, a key, and a method" do
            lambda { Puppet::RouteManager::Request.new }.should raise_error(ArgumentError)
        end

        it "should always convert the router name to a symbol" do
            Puppet::RouteManager::Request.new("ind", :method, "mykey").router_name.should == :ind
        end

        it "should use provided value as the key if it is a string" do
            Puppet::RouteManager::Request.new(:ind, :method, "mykey").key.should == "mykey"
        end
        ...
        it "should use the name of the provided instance as its key if an instance is provided as the key instead of a string" do
            ...
            request = Puppet::RouteManager::Request.new(:ind, :method, instance)
            request.key.should == "mykey"
            request.instance.should equal(instance)
        end

        it "should support options specified as a hash" do
            lambda { Puppet::RouteManager::Request.new(:ind, :method, :key, :one => :two) }.should_not raise_error(ArgumentError)
        end

        it "should support nil options" do
            lambda { Puppet::RouteManager::Request.new(:ind, :method, :key, nil) }.should_not raise_error(ArgumentError)
        end

        it "should support unspecified options" do
            lambda { Puppet::RouteManager::Request.new(:ind, :method, :key) }.should_not raise_error(ArgumentError)
        end

        it "should fail if options are specified as anything other than nil or a hash" do
            lambda { Puppet::RouteManager::Request.new(:ind, :method, :key, [:one, :two]) }.should raise_error(ArgumentError)
        end

        it "should use an empty options hash if nil was provided" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, nil).options.should == {}
        end

        it "should default to a nil node" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, nil).node.should be_nil
        end

        it "should set its node attribute if provided in the options" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, :node => "foo.com").node.should == "foo.com"
        end

        it "should default to a nil ip" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, nil).ip.should be_nil
        end

        it "should set its ip attribute if provided in the options" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, :ip => "192.168.0.1").ip.should == "192.168.0.1"
        end

        it "should default to being unauthenticated" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, nil).should_not be_authenticated
        end

        it "should set be marked authenticated if configured in the options" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, :authenticated => "eh").should be_authenticated
        end

        it "should keep its options as a hash even if a node is specified" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, :node => "eh").options.should be_instance_of(Hash)
        end

        it "should keep its options as a hash even if another option is specified" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, :foo => "bar").options.should be_instance_of(Hash)
        end

        it "should treat options other than :ip, :node, and :authenticated as options rather than attributes" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, :server => "bar").options[:server].should == "bar"
        end

        it "should normalize options to use symbols as keys" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, "foo" => "bar").options[:foo].should == "bar"
        end

        describe "and the request key is a URI" do
            describe "and the URI is a 'file' URI" do
                before do
                    @request = Puppet::RouteManager::Request.new(:ind, :method, "file:///my/file with spaces")
                end
            ...
            it "should set the protocol to the URI scheme" do
                Puppet::RouteManager::Request.new(:ind, :method, "http://host/stuff").protocol.should == "http"
            end

            it "should set the server if a server is provided" do
                Puppet::RouteManager::Request.new(:ind, :method, "http://host/stuff").server.should == "host"
            end

            it "should set the server and port if both are provided" do
                Puppet::RouteManager::Request.new(:ind, :method, "http://host:543/stuff").port.should == 543
            end
            ...
            it "should default to the masterport if the URI scheme is 'puppet'" do
                ...
                Puppet::RouteManager::Request.new(:ind, :method, "puppet://host/stuff").port.should == 321
            end

            it "should use the provided port if the URI scheme is not 'puppet'" do
                Puppet::RouteManager::Request.new(:ind, :method, "http://host/stuff").port.should == 80
            end

            it "should set the request key to the unescaped unqualified path from the URI" do
                Puppet::RouteManager::Request.new(:ind, :method, "http:///stuff with spaces").key.should == "stuff with spaces"
            end

            it "should set the :uri attribute to the full URI" do
                Puppet::RouteManager::Request.new(:ind, :method, "http:///stuff").uri.should == "http:///stuff"
            end
        end

        it "should allow indication that it should not read a cached instance" do
            Puppet::RouteManager::Request.new(:ind, :method, :key, :ignore_cache => true).should be_ignore_cache
        end
    ...
    it "should look use the Router class to return the appropriate router" do
        ind = mock 'router'
        Puppet::RouteManager::Router.expects(:instance).with(:myind).returns ind
        request = Puppet::RouteManager::Request.new(:myind, :method, :key)

        request.router.should equal(ind)
    end

    it "should use its router to look up the appropriate model" do
        ind = mock 'router'
        Puppet::RouteManager::Router.expects(:instance).with(:myind).returns ind
        request = Puppet::RouteManager::Request.new(:myind, :method, :key)
    ...
    it "should fail intelligently when asked to find a model but the router cannot be found" do
    ...
    it "should have a method for determining if the request is plural or singular" do
        Puppet::RouteManager::Request.new(:myind, :method, :key).should respond_to(:plural?)
    end

    it "should be considered plural if the method is 'search'" do
        Puppet::RouteManager::Request.new(:myind, :search, :key).should be_plural
    end

    it "should not be considered plural if the method is not 'search'" do
        Puppet::RouteManager::Request.new(:myind, :find, :key).should_not be_plural
    end

    it "should use its uri, if it has one, as its string representation" do
        Puppet::RouteManager::Request.new(:myind, :find, "foo://bar/baz").to_s.should == "foo://bar/baz"
    end

    it "should use its router name and key, if it has no uri, as its string representation" do
        Puppet::RouteManager::Request.new(:myind, :find, "key") == "/myind/key"
    end

    it "should be able to return the URI-escaped key" do
        Puppet::RouteManager::Request.new(:myind, :find, "my key").escaped_key.should == URI.escape("my key")
    end

    it "should have an environment accessor" do
        Puppet::RouteManager::Request.new(:myind, :find, "my key", :environment => "foo").should respond_to(:environment)
    end

    it "should set its environment to an environment instance when a string is specified as its environment" do
        Puppet::RouteManager::Request.new(:myind, :find, "my key", :environment => "foo").environment.should == Puppet::Node::Environment.new("foo")
    end

    it "should use any passed in environment instances as its environment" do
        env = Puppet::Node::Environment.new("foo")
        Puppet::RouteManager::Request.new(:myind, :find, "my key", :environment => env).environment.should equal(env)
    end

    it "should use the default environment when none is provided" do
        Puppet::RouteManager::Request.new(:myind, :find, "my key" ).environment.should equal(Puppet::Node::Environment.new)
    end

    it "should support converting its options to a hash" do
        Puppet::RouteManager::Request.new(:myind, :find, "my key" ).should respond_to(:to_hash)
    end

    it "should include all of its attributes when its options are converted to a hash" do
        Puppet::RouteManager::Request.new(:myind, :find, "my key", :node => 'foo').to_hash[:node].should == 'foo'
    end

    describe "when building a query string from its options" do
        before do
            @request = Puppet::RouteManager::Request.new(:myind, :find, "my key")
        end
