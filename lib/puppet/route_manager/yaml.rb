...
class Puppet::RouteManager::Yaml < Puppet::RouteManager::Repository
    ...
    def find(request)
        ...
        begin
            ...
            raise Puppet::Error, "Could not read YAML data for %s %s: %s" % [router.name, request.key, detail]
        end
        begin
            return from_yaml(yaml)
        rescue => detail
            raise Puppet::Error, "Could not parse YAML data for %s %s: %s" % [router.name, request.key, detail]
        end
    end
    ...
    def path(name)
        File.join(base, self.class.router_name.to_s, name.to_s + ".yaml")
    end

    # Do a glob on the yaml directory, loading each file found
    def search(request)
      Dir.glob(File.join(base, self.class.router_name.to_s, request.key)).collect { |f| YAML.load_file(f) }
    end
