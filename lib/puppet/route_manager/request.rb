...
class Puppet::RouteManager::Request
    ...
    # Look up the router based on the name provided.
    def router
        Puppet::RouteManager::Router.instance(router_name)
    end

    def router_name=(name)
        @router_name = name.to_sym
    end


    def model
        raise ArgumentError, "Could not find router '%s'" % router_name unless i = router
        i.model
    end
