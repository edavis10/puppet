...
class Puppet::RouteManager::REST < Puppet::RouteManager::Repository
    ...
    def find(request)
        return nil unless result = deserialize(network(request).get(router2uri(request), headers))
        result.name = request.key
        result
    end

    def search(request)
        unless result = deserialize(network(request).get(router2uri(request), headers), true)
            return []
        end
        return result
    end
    ...
    def destroy(request)
        ...
        deserialize network(request).delete(router2uri(request), headers)
    end

    def save(request)
        raise ArgumentError, "PUT does not accept options" unless request.options.empty?
        deserialize network(request).put(router2uri(request), request.instance.render, headers.merge({ "Content-Type" => request.instance.mime }))
    end
